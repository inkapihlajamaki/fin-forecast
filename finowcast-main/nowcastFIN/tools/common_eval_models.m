function[Loop,Eval] = common_eval_models(do_loop,Loop,Eval,xest,Par,t_m,m,country,datet,do_Covid,groups,n_fore)
...
%       o dirc_dropmissing [scalar 0/1] = switch on whether rounds whose target
%         quarter is not realised are dropped from the directional accuracy as
%         well as from the RMSE (1) or only from the RMSE (0, original toolbox)
...
if nargin < 12 || isempty(n_fore)
    n_fore = 1; % default = original toolbox behaviour
end
[C,hor_offsets,~] = common_horizons(n_fore);

% Default for the treatment of rounds without a realised target
if ~isfield(Eval,'dirc_dropmissing')
    Eval.dirc_dropmissing = 1;
end
...
for n_iter_mod = 1:Loop.n_iter
    ...
    for i=start_eval:end_eval
        ...
        % Iterate the AR(1) forward over all horizons
        % AR_path(hh) is the AR prediction for horizon C{hh}, AR_prev(hh) the AR
        % prediction for the quarter just before it (used for the direction)
        AR_path = nan(1,numel(C));
        AR_prev = nan(1,numel(C));
        AR_path(1) = AR_back;       % back-cast
        AR_prev(1) = AR_back_prev;
        for hh = 2:numel(C)
        ...
        for hh = 1:numel(C)
            ...
            Eval.(hor).Pred = [Eval.(hor).Pred; ...
                               t_m(i,1:2) datet(idx,1:2) AR_path(hh) Res.X_sm(idx,end) xest(idx,end)];
            Eval.(hor).Dirc = [Eval.(hor).Dirc; ...
                               t_m(i,1:2) datet(idx,1:2) AR_path(hh)>AR_prev(hh) ...
            ...
            if hh == 1 && BEQ_monOfQ(t_m(i,2)) >= Eval.gdp_rel
                Eval.(hor).Pred(end,6) = xest(idx,end);
                Eval.(hor).Dirc(end,6) = xest(idx,end)>xest(idx_prev,end);
            end
    ...
    for hh = 1:numel(C)
        ...
        % Keep only the rounds whose target quarter has been realised
        % NB: at horizons t+1 and beyond the last rounds of the evaluation sample
        %     have no realised target yet, so they cannot enter the accuracy
        %     metrics. The raw predictions written below are left untouched.
        valid_pred = all(isfinite(Eval.(hor).Pred(:,5:7)),2);
        if Eval.dirc_dropmissing == 1
            valid_dirc = valid_pred;
        else % original toolbox behaviour: rounds without a realised target still enter the FDA
            valid_dirc = true(size(Eval.(hor).Dirc,1),1);
        end
        P = Eval.(hor).Pred(valid_pred,:);
        Dc = Eval.(hor).Dirc(valid_dirc,:);
        Eval.(hor).n_eval = sum(valid_pred); % number of rounds entering the accuracy metrics
        if Eval.(hor).n_eval == 0
            warning(['No realised target for horizon ',hor,'. Accuracy metrics will be NaN. ' ...
                     'Consider setting an earlier end date for the evaluation (Eval.eval_endyear / Eval.eval_endmonth).'])
        end
        ...
        % Evaluate the RMSFE
        Eval.(hor).RMSFE_AR1_all = sqrt(mean((P(:,5) - P(:,7)).^2));
        Eval.(hor).RMSFE_mod_all = sqrt(mean((P(:,6) - P(:,7)).^2));
        temp_postCovid = P(:,3)>2020;
        Eval.(hor).RMSFE_AR1_postCovid = sqrt(mean((P(temp_postCovid,5) - P(temp_postCovid,7)).^2));
        Eval.(hor).RMSFE_mod_postCovid = sqrt(mean((P(temp_postCovid,6) - P(temp_postCovid,7)).^2));  
        temp_preCovid = P(:,3)<2020;
        Eval.(hor).RMSFE_AR1_preCovid = sqrt(mean((P(temp_preCovid,5) - P(temp_preCovid,7)).^2));
        Eval.(hor).RMSFE_mod_preCovid = sqrt(mean((P(temp_preCovid,6) - P(temp_preCovid,7)).^2));
        temp_Covid = P(:,3)==2020;
        Eval.(hor).RMSFE_AR1_Covid = sqrt(mean((P(temp_Covid,5) - P(temp_Covid,7)).^2));
        Eval.(hor).RMSFE_mod_Covid = sqrt(mean((P(temp_Covid,6) - P(temp_Covid,7)).^2));
        temp_noCovid = P(:,3)~=2020;
        Eval.(hor).RMSFE_AR1_noCovid = sqrt(mean((P(temp_noCovid,5) - P(temp_noCovid,7)).^2));
        Eval.(hor).RMSFE_mod_noCovid = sqrt(mean((P(temp_noCovid,6) - P(temp_noCovid,7)).^2));
        ...
        for dd = 1:numel(D)
            mth = D{dd};
            temp_D = P(mod(P(:,2),3)==mod(dd,3),:); % Select only values corresponding to 1st, 2nd, or 3rd month
        ...
        Eval.(hor).FDA_AR1_all = 1-mean(abs(Dc(:,5)-Dc(:,7)));
        Eval.(hor).FDA_mod_all = 1-mean(abs(Dc(:,6)-Dc(:,7)));
        temp_postCovid = Dc(:,3)>2020;
        Eval.(hor).FDA_AR1_postCovid = 1-mean(abs(Dc(temp_postCovid,5)-Dc(temp_postCovid,7)));
        Eval.(hor).FDA_mod_postCovid = 1-mean(abs(Dc(temp_postCovid,6)-Dc(temp_postCovid,7)));
        temp_preCovid = Dc(:,3)<2020;
        Eval.(hor).FDA_AR1_preCovid = 1-mean(abs(Dc(temp_preCovid,5)-Dc(temp_preCovid,7)));
        Eval.(hor).FDA_mod_preCovid = 1-mean(abs(Dc(temp_preCovid,6)-Dc(temp_preCovid,7)));
        temp_Covid = Dc(:,3)==2020;
        Eval.(hor).FDA_AR1_Covid = 1-mean(abs(Dc(temp_Covid,5)-Dc(temp_Covid,7)));
        Eval.(hor).FDA_mod_Covid = 1-mean(abs(Dc(temp_Covid,6)-Dc(temp_Covid,7)));
        temp_noCovid = Dc(:,3)~=2020;
        Eval.(hor).FDA_AR1_noCovid = 1-mean(abs(Dc(temp_noCovid,5)-Dc(temp_noCovid,7)));
        Eval.(hor).FDA_mod_noCovid = 1-mean(abs(Dc(temp_noCovid,6)-Dc(temp_noCovid,7)));

        % Evaluate the FDA at different months
        for dd = 1:numel(D)
            mth = D{dd};
            temp_D = Dc(mod(Dc(:,2),3)==mod(dd,3),:); % Select only values corresponding to 1st, 2nd, or 3rd month
